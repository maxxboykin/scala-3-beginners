...
object MethodNotation_Max {
  ...
  class Person(val name: String, val age: Int, favoriteMovie: String) {
    ...
    infix def !!(progLanguage: String): String = s"$name wonders how can $progLanguage be so cool!" // ?, !, >>, <+>
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(2 + 3)
    println(2.+(3)) // exactly the same.
    // an operator in scala is just a method !!!!
    println(mary !! "Scala")
    ...
    //postfix notation
    println(mary.isAlive) // stick to dot
    println(mary isAlive) // discouraged - brings little benefit and confusing
